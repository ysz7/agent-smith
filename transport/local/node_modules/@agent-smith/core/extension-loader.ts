...
export class ExtensionLoader {
  ...
  private discoveredNames: string[] = []
  ...
  private async loadFromDir(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      // Track discovered regardless of enabled state
      if (!this.discoveredNames.includes(entry)) {
        // Only add if it has a loadable index
        const indexPath = path.join(dir, entry, 'dist', 'index.js')
        const indexFallback = path.join(dir, entry, 'index.js')
        const exists = await fs.access(indexPath).then(() => true).catch(() => false)
          || await fs.access(indexFallback).then(() => true).catch(() => false)
        if (exists) this.discoveredNames.push(entry)
      }
