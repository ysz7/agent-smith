...
- [ ] Протестировать установку на macOS без sudo
...
## Фаза 5 — Мульти-агент (после MVP)

- [ ] Написать `AgentTeam` (`core/agent-team.ts`) — управление командой
- [ ] Написать `Orchestrator` (`core/orchestrator.ts`) — раздача задач
- [ ] Поддержка создания агентов пользователем через чат
- [ ] Поддержка динамического создания агентов оркестратором
- [ ] UI для управления командой агентов
- [ ] Визуализация общения между агентами в UI
- [ ] Конфиг communication — кто с кем может общаться

---

## Фаза 6 — AWS (после мульти-агента)

- [ ] Написать `DynamoStorage` (`transport/cloud/storage.ts`)
- [ ] Написать `LambdaTransport` (`transport/cloud/gateway.ts`)
- [ ] Написать `EventBridgeScheduler` (`transport/cloud/scheduler.ts`)
- [ ] CDK стек для деплоя
- [ ] Команда `agent-smith deploy --aws`
- [ ] Документация по деплою

---

## Фаза 7 — Экосистема (долгосрочно)

- [ ] Маркетплейс скиллов (как ClawHub)
- [ ] Совместимость со скиллами из OpenClaw
- [ ] Telegram канал как опциональный транспорт
- [ ] Mobile приложение (React Native)
- [ ] Документация для разработчиков скиллов

---

## Фаза 8 — Долгосрочная память (после MVP)

> Базовая память через JSON файл уже есть в MVP (`~/.agent-smith/memory.json`).
> Этот раздел — улучшения которые добавляем когда MVP готов и протестирован.

**8.1 — SQLite (когда фактов > 1000)**
- [ ] Заменить `memory.json` на SQLite через интерфейс `IStorage` — ядро агента не меняется
- [ ] Индексы для быстрого поиска по тегам и дате
- [ ] Миграция существующих данных из JSON в SQLite автоматически

**8.2 — Семантический поиск (когда нужен умный поиск)**
- [ ] Интегрировать векторную БД (LanceDB — работает локально, не требует сервера)
- [ ] Генерировать embeddings для каждого факта при сохранении
- [ ] Искать по смыслу а не по ключевым словам
- [ ] Пример: "что я говорил про работу?" находит всё связанное с работой

**8.3 — Общая память для нескольких агентов**
- [ ] Агенты в команде делят общую память
- [ ] Neo нашёл вакансию → сохранил → Morpheus видит это в своей памяти
- [ ] Разграничение: личная память агента vs общая память команды

**8.4 — UI для памяти**
- [ ] Раздел "Memory" в настройках — список всех сохранённых фактов
- [ ] Поиск по фактам
