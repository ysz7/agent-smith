...
**Два типа агентов — полностью независимы:**
- **User agents** — создаются пользователем вручную через UI/чат. Персистентны, живут до явного удаления.
- **Orchestrator agents** — создаются главным агентом автоматически для выполнения задачи. Временные, привязаны к конкретному запросу.

**Разграничение полномочий:**
- Оркестрация (право главного агента создавать суб-агентов) включается отдельным разрешением в Settings. По умолчанию — выключено.
- Пользователь в любой момент может остановить любого агента созданного оркестратором прямо из UI, не затрагивая user agents.
- User agents не могут быть остановлены или удалены оркестратором — только пользователем вручную.
