...
- [ ] Добавить отдельную вкладку AgentsOffice где показывать всех существующих агентов в интерфейсе в видео офиса. можешь использовать 3d либо 2d библиотеки. визуально показывать что они делают и где находятся в офисе. так же показывать если они комуницируют друг с другом. добавить функционал чтобы можно было в ручную удалять агента либо добавлять нового через ui. сделать такой фукнционал чтобы пользователь мог запретить использовать субагентов для главного агента. но так же чтобы с суб агентами можна было общаться и работать отдельно от главного. таким образом у нас будет двойной функционал.
...
**9.4 — Calendar**
- [ ] Google Calendar через OAuth2 — чтение и создание событий
- [ ] Apple Calendar (macOS — CalDAV)
- [ ] Естественный язык: "запланируй встречу в пятницу после обеда"
